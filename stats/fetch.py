...
def load_competitions():
    ...
    for competition in comp_json['competitions']:
        ...
        obj, created = Competition.objects.get_or_create(api_id=api_id)
...
def load_teams(season_id):
    ...
    teams=[]
    season_obj = Season.objects.get(api_id=season_id)
...
def _load_team_players(tid):
    ...
    team_obj = Team.objects.get(api_id=tid)
    players=[]
    ...
    for player in players_list:
        ...
        obj, created = Player.objects.get_or_create(api_id=api_id)
    ...
    if manager:
        ...
        manager_obj, man_created = Player.objects.get_or_create(api_id=api_id)
    ...
    if venue:
        api_id = venue.get('id', '')
        venue_obj, ven_created = Venue.objects.get_or_create(api_id=api_id)
    ...
    for summary in season_json['summaries']:
        ...
        for competitor in totals.get('competitors', []):
            for player in competitor.get('players'):
                ...
                player_obj = Player.objects.get_or_create(api_id=player_id, name=player_name, defaults={'team_id': Team.objects.get(api_id=competitor.get('id'))})
                stat_obj, stat_created = Statistic.objects.get_or_create(
